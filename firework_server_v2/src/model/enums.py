...
class cureTypeEnum(enum.Enum):
    chem = "Химеотерапия"
    ray = "Лучевая терапия"
    surgery = "Операция"
    target = "Таргетная терапия"
    hormon = "Гормонотерапия"
    palliative = "Паллиативное лечение"
    def __str__(self):
        if self is not None:
            return self.value
        else:
...
class recomendationTypeEnum(enum.Enum):
    info = "Информацию о том, как справляться с эмоциями"
    exercises = "Упражнения, чтобы самому приводить себя в норму"
    support = "Группу поддержки"
    work = "Индивидуальную работу с психологом"
    other = "Другое"
    def __str__(self):
        if self is not None:
            return self.value
